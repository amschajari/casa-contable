...
const AdminLayout = ({ children }) => {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#0f172a] transition-colors duration-300">
            {/* Header / Navbar */}
            <nav className={`sticky top-0 z-[100] border-b transition-colors duration-300 ${isDarkMode ? 'bg-[#1e293b]/80 border-white/5 backdrop-blur-md' : 'bg-white/80 border-slate-200 backdrop-blur-md'
                }`}>
                <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-8">
                        <Link to="/admin/dashboard" className={`text-xl font-black italic tracking-tighter ${isDarkMode ? 'text-white' : 'text-slate-900'
                            ...
                            Casa<span className="text-indigo-600">Contable</span>
                        </Link>
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="hidden md:flex bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-600/20 active:scale-95 items-center gap-2 mr-2"
                        >
            ...
            <button
                onClick={() => setIsModalOpen(true)}
                className="md:hidden fixed bottom-24 right-6 z-[110] bg-indigo-600 text-white p-4 rounded-3xl shadow-2xl shadow-indigo-600/40 active:scale-90 transition-transform flex items-center justify-center border-4 border-white dark:border-[#0f172a]"
            >
            ...
            <div className={`md:hidden fixed bottom-0 left-0 right-0 z-[100] p-4 pb-8 transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a]/80 backdrop-blur-lg border-t border-white/5' : 'bg-white/80 backdrop-blur-lg border-t border-slate-200'
                ...
                <div className="flex justify-around items-center max-w-md mx-auto">
                    <Link to="/admin/dashboard" className={`p-4 rounded-2xl transition-all ${isActive('/admin/dashboard') ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'text-slate-400 hover:text-indigo-500'
                        }`}>
                        <Home className="w-6 h-6" />
                    </Link>
                    <div className="w-16"></div> {/* Espacio para el FAB */}
                    <Link to="/admin/movements" className={`p-4 rounded-2xl transition-all ${isActive('/admin/movements') ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'text-slate-400 hover:text-indigo-500'
                        }`}>
